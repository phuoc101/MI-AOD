...
def single_gpu_extract(model, data_loader, show=False):
    model.eval()
    feats = None
    dataset = data_loader.dataset
    prog_bar = mmcv.ProgressBar(len(dataset))
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            data.update({"x": data.pop("img")})
            feat = model.extract_feat(data["x"])
        if feats is None:
            feats = torch.zeros([len(data_loader), feat.shape[0]])
            feats[0, :] = feat
        else:
            feats[i, :] = feat
        batch_size = len(data["img_metas"][0].data)
...
def collect_y_heads_gpu(y_head_part, size):
    ...
    part_tensor = torch.tensor(bytearray(pickle.dumps(y_head_part)), dtype=torch.uint8, device="cuda")
